...
maxp = max(max(accuracy_values));
p = 0;
l = 0;
for i = 1:length(lambda_values)
    for k = 1:length(sigma_values)
        if accuracy_values(i,k) == maxp
            p = p +1;
            mat(p,1) = lambda_values(i);
            mat(p,2) = sigma_values(k);
        end
    end
end
...
title('Valeur de lambda maximisant le pourcentage de bonnes classifications en fonction de sigma');
