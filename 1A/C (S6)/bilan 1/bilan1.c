...
void create(string *string_dest, char *chaine_src){
    int i;
    string_dest ->  str = malloc(sizeof(char) * (strlen(chaine_src) + 1));
    string_dest -> N = strlen(chaine_src) + 1;
    
    for ( i = 0; i < strlen(chaine_src) - 1; i++)
    {
...
void add(string *chaine_dest, char c){
    chaine_dest->str = realloc(chaine_dest->str, sizeof(char) * (chaine_dest->N + 1));
