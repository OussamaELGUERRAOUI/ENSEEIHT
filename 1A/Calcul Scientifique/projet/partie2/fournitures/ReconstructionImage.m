...
%[ V1, D1, n_ev, itv, flag ] = power_v12( M, search_space, percentage, eps, maxit )
[ V1, D1, it, flag ] = subspace_iter_v0( M, search_space,eps, maxit )
% [ V1, D1, n_ev, it, itv, flag ] = subspace_iter_v1( M, search_space, percentage, eps, maxit )
%[ V1, D1, n_ev, it, itv, flag ] = subspace_iter_v2( M, search_space, percentage, puiss, eps, maxit )
%[ V1, D1, n_ev, it, itv, flag ] = subspace_iter_v3( M, search_space, percentage, puiss, eps, maxit )
%%
...
% vecteur pour stocker la différence entre l'image et l'image reconstuite
inter1 = 1:240:length(V1);
inter1(end) = length(V1);
...
%for k = inter1
   ...
   % ti1 = ti1+1;
    ...
    % Calcul de la différence entre les 2 images
    %td1 = td1 + 1;
    %differenceSVD1(td) = sqrt(sum(sum((I-Im_k1).^2)));
    %pause
%end
...
figure ;
hold on 
plot(length(differenceSVD1), differenceSVD1, 'rx')
xlim([0  200]);
ylabel('RMSE')
xlabel('rank k')
pause
