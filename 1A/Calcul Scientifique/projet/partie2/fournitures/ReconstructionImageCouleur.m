...
I = double(I);
% diviser les canals des trois couleurs principal
[R, V, B] = imsplit(I);
...
for k = inter
    ...
    Ir =uint8(Ur(:, 1:k) * Sr(1:k, 1:k) * Vr(:, 1:k)');
    Iv =uint8(Ug(:, 1:k) * Sg(1:k, 1:k) * Vg(:, 1:k)');
    Ib =uint8(Ub(:, 1:k) * Sb(1:k, 1:k) * Vb(:, 1:k)');
    ...
    td = td + 1;
    %differenceSVDr(td) = sqrt(sum(sum((Ir - double(R)).^2)));
    %differenceSVDg(td) = sqrt(sum(sum((Iv - double(V)).^2)));
    %differenceSVDb(td) = sqrt(sum(sum((Ib - double(B)).^2)));
    pause
end
...
[ V1, D1, n_ev1, it1, itv1, flag1 ] = subspace_iter_v3(A, search_space, percentage, puiss, eps, maxit);
[ V2, D2, n_ev2, it2, itv2, flag2 ] = subspace_iter_v3(B, search_space, percentage, puiss, eps, maxit);
[ V3, D3, n_ev3, it3, itv3, flag3 ] = subspace_iter_v3(C, search_space, percentage, puiss, eps, maxit);
