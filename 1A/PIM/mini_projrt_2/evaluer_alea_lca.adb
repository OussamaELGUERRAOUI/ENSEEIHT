...
procedure Evaluer_Alea_LCA is
   ...
   begin
	...
	-- le bon.
   procedure Calculer_Statistiques (Borne: in Integer;
                                    Taille   : in Integer;
                                    Min: out Integer;
                                    Max: out Integer
     ) with
	...
	begin
      ...
      Max := 0;
		for I in 1..Taille loop
		    ...
		    if not (Cle_Presente(SdA, Number)) then
			    Enregistrer(Sda, Number,1);
		    else
			    Enregistrer(Sda, Number, La_Donnee(Sda,Number) + 1);
		    end if;
		end loop;
		for i in 1..Borne loop
			if La_Donnee (Sda, i) < Min then
				Min := La_Donnee (Sda, i);
			else Null;
			end if;
			if La_Donnee (Sda, i) > Max then
				Max := La_Donnee (Sda, i);
			else Null;
			end if;
