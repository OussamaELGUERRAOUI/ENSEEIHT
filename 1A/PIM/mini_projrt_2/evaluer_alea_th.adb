   ...
   procedure Calculer_Statistiques (
	...
	is
		...
		package TH_Integer_Integer is
			new TH (T_Cle => Integer, T_Donnee => Integer, Capacity => Taille ,traiter_cle_hachage => fonction_hachage);
	...
	begin
      ...
      Max := 0;
		for I in 1..Taille loop
		    ...
		    if not (Tcle_Present(Sda_tab , number)) then
			    Tenregistrerr(Sda_tab, Number,1);
		    else
			    Tenregistrerr(Sda_tab , Number, Tla_Donnee(Sda_tab, Number) + 1);
		    end if;
		end loop;
		for i in 1..Borne loop
			if Tla_Donnee (Sda_tab, i) < Min then
				Min := Tla_Donnee (Sda_tab , i);
			else Null;
			end if;
			if Tla_Donnee (Sda_tab , i) > Max then
				Max := Tla_Donnee (Sda_tab, i);
			else Null;
			end if;
