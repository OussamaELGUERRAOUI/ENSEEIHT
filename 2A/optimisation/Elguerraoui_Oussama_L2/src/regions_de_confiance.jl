...
function regions_de_confiance(f::Function, gradf::Function, hessf::Function, x0::Vector{<:Real};
    ...
    normGradf0 = norm(gradf(x0))
    Δk = Δ0

    if norm(gradf(x0)) <= max(tol_rel*normGradf0 , tol_abs)
        flag = 0
    end
    ...
    while flag == -1
        ...
        if algo_pas == "gct"
            sk = gct(gk, Hk, Δk; tol_abs = tol_abs, tol_rel=tol_rel,max_iter = max_iter_gct)
        else
            sk = cauchy(gk, Hk, Δk; tol_abs=tol_abs)
        end
       

        mk0 = f(x_sol)
        mk = f(x_sol) + gk'*sk + (1/2)*sk'*Hk*sk
        
        phik = (f(x_sol) - f(x_sol + sk))/(mk0 - mk)

        if phik >= η1
            x_sol = x_sol + sk          # xk+1 = xk + sk
            xs = vcat(xs, [x_sol])
            xk = xs[end-1]
           if norm(x_sol - xk) <= epsilon*max(tol_rel*norm(xk), tol_abs)
            flag = 1
        ...
        if norm(gradf(x_sol)) <= max(tol_rel*normGradf0 , tol_abs)
            flag = 0
            break
