    ...
    private static int computeMax(ExecutorService executor, int[] array, int numberOfTasks) throws InterruptedException, ExecutionException {
        ...
        for (int i = 0; i < numberOfTasks; i++){
            if (i == numberOfTasks - 1){
                coefficient = Math.min(array.length, (i+1)*taskSize);
            }
            else{
                coefficient = (i+1)*taskSize;
            }
            PartialMax job = new PartialMax(array, i*taskSize, coefficient);
        ...
        return max; // pour que ça compilefoo
    }
