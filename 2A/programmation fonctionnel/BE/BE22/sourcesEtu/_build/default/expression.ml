...
let rec eval_bienformee arb =
  if (bienforme arb) then
    match arb with 
     | Naire(op,t ::q) -> (match op with 
                          | Plus -> List.fold_left (fun y x -> y + (eval_bienformee x)) 0 l
                          | Moins -> (eval_bienformee t) - (eval_bienformee List.hd q) 
                          | Mult -> List.fold_left (fun y x -> y * (eval_bienformee x)) 1 l
                          | Div -> (eval_bienformee t)/(eval_bienformee List.hd q) )
     | Entier a -> a
  else 
    failwith "Malexprimé"
